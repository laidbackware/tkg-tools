...
function imgpkg_copy() {
    ...
    case $flags in # Set separate output directories for download type
      -i)
        output_dir=$IMAGE_LOCATION/vmware-images/images
        ;;
      -b)
        output_dir=$IMAGE_LOCATION/vmware-images/bundles
        ;;
    esac
...
for file_path in $IMAGE_LOCATION/images/*; do
  filename=$(basename -- "$file_path")
  echo -e "$filename\n"
  image_name=${filename/"harbor.unknown_tkg-base_"/"$TKG_CUSTOM_IMAGE_REPOSITORY/"}
  image_name=${image_name//"_"/"/"}
  echo -e "$image_name \n"
  imgpkg push -i $image_name -f $file_path
done
